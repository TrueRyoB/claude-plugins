...
`<name>` = 反映するローカル skill 名。省略時は手順 1 で差分検出して対象を洗い出す。

## 手順

1. 差分検出（1 コマンド。CHANGED/NEW が対象）:
...
3. 同期: 単一ファイルは `cp ~/.claude/skills/<name>/SKILL.md $REPO/plugins/analysis-skills/skills/<name>/SKILL.md`。サブファイルありは `rsync -a --delete --exclude 'LEDGER.md' ~/.claude/skills/<name>/ $REPO/plugins/analysis-skills/skills/<name>/`。新規は dir 作成後に同期。
   **ローカル限定ファイル**: `LEDGER.md`（是正台帳）は社外秘の事例を含み得るため **公開 repo へ絶対に同期しない**（repo は PUBLIC）。同期後に `git status` で意図しないファイルが混入していないか確認する。
